...
<body>
  <section id="nav-bar" style="padding: 30px; font-family: 'Montserrat', sans-serif;">
  ...
  <section class="user_profile" style="font-family: 'Montserrat', sans-serif;">
    <?php
    ...
    function update()
    {
      if (isset($_POST['update'])) {
        ...
        if ($pwd == $cpwd) {
          ...
          $query = "UPDATE users SET name = '$name', password = '$pwd', dob = '$dob'  WHERE uname = '{$_SESSION['uname']}'";
          $results = mysqli_query($db, $query);
    ...
    <div class="profile">
      <br>
      <h1 align="center">Edit Your profile</h1><br>
      <form class="change-profile" method="post">
        <?php update(); ?>
        <label for="name">Name</label><input type="text" class="input-group input-group-sm" name="name" required value="<?php if (isset($_SESSION['uname'])) {
                                                                                                                          echo $row['name'];
                                                                                                                        } ?>"><br>
        <label for="email">Email ID</label><input disabled type="email" class="input-group input-group-sm" name="email" value="<?php if (isset($_SESSION['uname'])) {
                                                                                                                                  echo $row['email'];
                                                                                                                                } ?>"><br>
        <label for="dob">DOB</label><input type="date" class="input-group input-group-sm" name="dob" required value="<?php if (isset($_SESSION['uname'])) {
                                                                                                                        echo $row['dob'];
                                                                                                                      } ?>"><br>
